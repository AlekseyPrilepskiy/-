...
namespace ConsoleApp2
{
    ...
    class Message
    {
        ...
        public void ShowAllFighters(List<IDamageable> characters)
        {
    ...
    class Arena
    {
        protected List<IDamageable> Characters;
        protected List<IDamageable> Fighters;
        protected Message Message = new Message();
        protected IDamageable FirstFighter;
        protected IDamageable SecondFighter;

        public Arena(List<IDamageable> characters)
        {
            Characters = characters;
            Message.ShowAllFighters(Characters);
        }

        public void IsBattle()
        {
            Fighters = ChooseFigthers();
            FirstFighter = Fighters[0];
            SecondFighter = Fighters[1];

            while (FirstFighter.Health > 0 && SecondFighter.Health > 0)
            {
                SecondFighter.TakeDamage(FirstFighter.CalculateDamage());
                Console.WriteLine($"У второго бойца осталось {SecondFighter.Health} хп");

                Console.ReadLine();

                if (SecondFighter.Health != 0)
                {
                    FirstFighter.TakeDamage(SecondFighter.CalculateDamage());
                    Console.WriteLine($"У первого бойца осталось {FirstFighter.Health} хп");
        ...
        private List<IDamageable> ChooseFigthers()
        {
            ...
            IDamageable fighter1 = Characters[userChoice1 - 1].Clone();
            IDamageable fighter2 = Characters[userChoice2 - 1].Clone();
